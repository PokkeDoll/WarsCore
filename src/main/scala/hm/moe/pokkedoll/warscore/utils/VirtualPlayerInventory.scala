...
object VirtualPlayerInventory {
  def to(inv: VirtualPlayerInventory): String = {
    if(inv == null) " " else {
  ...
  def from(string: String): VirtualPlayerInventory = {
    val inv = new VirtualPlayerInventory
    val yaml = new YamlConfiguration
    try {
